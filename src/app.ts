type Mode = "time" | "quote" | "meteor" | "zen";
type SessionStatus = "idle" | "running" | "finished";
type WordResult = "good" | "bad";
...
interface UiRefs {
  ...
  promptZone: HTMLElement;
  meteorZone: HTMLElement;
  typeInput: HTMLInputElement;
  startBtn: HTMLButtonElement;
  restartBtn: HTMLButtonElement;
  newSeedBtn: HTMLButtonElement;
  durationSelect: HTMLSelectElement;
  dictionaryPack: HTMLSelectElement;
  optPunctuation: HTMLInputElement;
  optNumbers: HTMLInputElement;
  optLowercase: HTMLInputElement;
  optCustomOnly: HTMLInputElement;
  customWords: HTMLTextAreaElement;
  saveCustomBtn: HTMLButtonElement;
  clearCustomBtn: HTMLButtonElement;
  customStatus: HTMLElement;
  report: HTMLElement;
  statWpm: HTMLElement;
  statRaw: HTMLElement;
  statAcc: HTMLElement;
  statStreak: HTMLElement;
  statErrors: HTMLElement;
  statLives: HTMLElement;
}
...
interface GameState {
  ...
  visibleStart: number;
  visibleEnd: number;
  visibleEls: Map<number, HTMLSpanElement>;
  previewRaf: number;
  pendingPreviewValue: string;
  lastPreviewValue: string;
  startTime: number;
  finishTime: number;
  hudRaf: number;
  lastHudPaint: number;
  ...
  meteorRaf: number;
  meteorLastTs: number;
  meteorLockRaf: number;
  meteorPendingInput: string;
  meteorLastLockInput: string;
  meteorLockedId: number | null;
}
...
const ui: UiRefs = {
  modeCards: [...document.querySelectorAll<HTMLButtonElement>(".mode-card")],
  modeCaption: byId<HTMLElement>("mode-caption"),
  modeChip: byId<HTMLElement>("mode-chip"),
  phaseLabel: byId<HTMLElement>("phase-label"),
  timerLabel: byId<HTMLElement>("timer-label"),
  promptZone: byId<HTMLElement>("prompt-zone"),
  meteorZone: byId<HTMLElement>("meteor-zone"),
  typeInput: byId<HTMLInputElement>("type-input"),
  startBtn: byId<HTMLButtonElement>("start-btn"),
  restartBtn: byId<HTMLButtonElement>("restart-btn"),
  newSeedBtn: byId<HTMLButtonElement>("new-seed-btn"),
  durationSelect: byId<HTMLSelectElement>("duration-select"),
  dictionaryPack: byId<HTMLSelectElement>("dictionary-pack"),
  optPunctuation: byId<HTMLInputElement>("opt-punctuation"),
  optNumbers: byId<HTMLInputElement>("opt-numbers"),
  optLowercase: byId<HTMLInputElement>("opt-lowercase"),
  optCustomOnly: byId<HTMLInputElement>("opt-custom-only"),
  customWords: byId<HTMLTextAreaElement>("custom-words"),
  saveCustomBtn: byId<HTMLButtonElement>("save-custom-btn"),
  clearCustomBtn: byId<HTMLButtonElement>("clear-custom-btn"),
  customStatus: byId<HTMLElement>("custom-status"),
  report: byId<HTMLElement>("session-report"),
  statWpm: byId<HTMLElement>("stat-wpm"),
  statRaw: byId<HTMLElement>("stat-raw"),
  statAcc: byId<HTMLElement>("stat-acc"),
  statStreak: byId<HTMLElement>("stat-streak"),
  statErrors: byId<HTMLElement>("stat-errors"),
  statLives: byId<HTMLElement>("stat-lives"),
};
...
const state: GameState = {
  ...
  visibleStart: 0,
  visibleEnd: 0,
  visibleEls: new Map<number, HTMLSpanElement>(),
  previewRaf: 0,
  pendingPreviewValue: "",
  lastPreviewValue: "",
...
async function hydrateLargeDictionary(): Promise<void> {
  try {
...
function setMode(mode: string): void {
  ...
  ui.modeCards.forEach((card) => {
    card.classList.toggle("active", card.dataset.mode === safeMode);
  });

  ui.modeCaption.textContent = MODE_INFO[safeMode];
  ui.modeChip.textContent = safeMode.toUpperCase();
  ui.report.classList.add("hidden");
  ui.phaseLabel.textContent = "Ready";

  const meteor = safeMode === "meteor";
...
function resetSession(newPrompt: boolean): void {
  stopSessionLoops();
  clearMeteor();
  cancelPreviewFrame();
...
function hudTick(ts: number): void {
  if (state.status !== "running") {
    return;
  }
...
function persistBest(wpm: number): void {
...
function renderReport(summary: Summary, reason: string): void {
...
function updateCurrentWordPreview(typed: string): void {
  if (state.mode === "meteor") {
    return;
  }
...
function appendPromptWords(amount: number): void {
...
function updatePromptAfterCommit(prevIndex: number, wasPerfect: boolean): void {
  const words = activeWords();
...
function renderPromptWindow(force: boolean): void {
  if (state.mode === "meteor") {
    return;
  }
  ...
  const nextVisibleMap = new Map<number, HTMLSpanElement>();
...
function paintCompletedWord(el: HTMLSpanElement, word: string, isGood: boolean): void {
  el.className = `prompt-word ${isGood ? "done-good" : "done-bad"}`;
  el.textContent = word;
}

function paintFutureWord(el: HTMLSpanElement, word: string): void {
  el.className = "prompt-word future";
  el.textContent = word;
}

function paintCurrentWord(el: HTMLSpanElement, word: string, typed: string): void {
  el.className = "prompt-word current";
  el.innerHTML = renderCurrentWord(word, typed);
}

function renderCurrentWord(target: string, typed: string): string {
  const chars = [];
...
function compareWords(typed: string, target: string): { correct: number; wrong: number } {
  let correct = 0;
  let wrong = 0;
...
function scheduleMeteorLockUpdate(force = false): void {
  if (state.mode !== "meteor") {
    return;
  }
...
function updateMeteorLocks(typed: string): void {
  if (state.mode !== "meteor") {
    return;
  }
...
function maybePunctuate(word: string): string {
  const marks = [".", ",", "!", "?", ":", ";"];
  if (Math.random() < 0.25) {
